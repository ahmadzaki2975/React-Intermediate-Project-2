...
import { getArchivedNotes, getUserLogged } from "../utils/api";
          ...
          return (
            ...
            >
              <div className="user-data m-5 text-lg bg-blue-500 rounded p-2 text-white">
                ...
                <div
                  className="bg-red-700 font-semibold w-min px-3 py-1 rounded mt-2 cursor-pointer hover:bg-red-600"
                  title="Logout"
                  onClick={() => {
                    setUser(null);
                    navigate("/");
              ...
              <h1 className={style}>
                {lang === "en" ? "Archived Notes" : "Catatan yang diarsipkan"}
              </h1>
              <div className="grid grid-cols-1 px-5 gap-1 pb-20">
                {notes !== null ? (
                  notes.length > 0 ? (
                    notes.map((note) => {
